...
df = pd.read_csv('05_Auswertung/AllInOne_HR_Upscaling_results_with_NIQE.csv')
...
df_filtered = df[(df['Width'] >= min_width_threshold) & (df['Height'] >= min_height_threshold) & (df['NIQE'] >= 0)]
...
for df_set, label in sets:
    ...
    for metric, color in zip(metrics, colors):
        ...
        z_scores = zscore(df_set[metric])
        outliers = np.abs(z_scores) > 300

        # Mark outliers in red and the rest in blue
        ax.scatter(df_set['Area'][~outliers], df_set[metric][~outliers], label=f'{metric}', s=1, color=color)
        ax.scatter(df_set['Area'][outliers], df_set[metric][outliers], label=f'{metric} - OUTLIERS', color='red', s=1)

        # Perform linear regression on the non-outlier data
        slope, intercept, r_value, p_value, std_err = linregress(df_set['Area'][~outliers], df_set[metric][~outliers])
        regression_line = slope * df_set['Area'] + intercept
        ax.plot(df_set['Area'][~outliers], regression_line[~outliers], color='gray', linestyle='--', linewidth=2)

    ax.set_ylim(y_range)  # Set the y-range to the fixed values
    ax.set_title(f'Metrics - {label}')
